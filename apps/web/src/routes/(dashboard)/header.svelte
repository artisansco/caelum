				...
				<Popover sameWidth={false}>
					{#snippet children(popover)}
						<button {...popover.trigger}>
							<Avatar src="https://placehold.co/400?text=Acme">
								{#snippet children(avatar)}
									<img {...avatar.image} alt="Acme" loading="lazy" class="size-10 rounded-full" />
									<span {...avatar.fallback} class="text-xs">Acme</span>
								{/snippet}
							</Avatar>
						</button>
